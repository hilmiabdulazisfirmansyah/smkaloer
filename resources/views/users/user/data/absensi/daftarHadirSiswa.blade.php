<div class="box">
			...
			<table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
				...
				<tbody>
					...
					<tr role="row" class="odd">
						...
						@php
						$nama_siswa = DB::table('users')->find($kehadiran->user_id)->name;
						@endphp
						...
						@if ($users == 'Guru' && $job_title == '')
						    ...
						    @case('Belum Di Verifikasi')
						        <span id="siswa-{{ $kehadiran->user_id }}" class="badge bg-red">Belum Di Verifikasi</span>
						    @break

						    @case('Sudah Di Verifikasi')
						        <span id="siswa-{{ $kehadiran->user_id }}" class="badge bg-green">Sudah Di Verifikasi</span>
						    @break
						...
						<td class="text-center">
							...
							@if ($users == 'Guru' && $job_title == '')
								@switch($kehadiran->verifikasi_guru)
							    @case('Belum Di Verifikasi')
							        <button id="verifikasi" class="btn bg-green" data-id="{{ $kehadiran->user_id }}"> <span class="glyphicon glyphicon-ok"> </span> Verifikasi</button>
							     @break

							      @case('Sudah Di Verifikasi')
							        <button id="verifikasi" class="btn bg-red" data-id="{{ $kehadiran->user_id }}"> <span class="glyphicon glyphicon-remove"> </span> Batalkan</button>
							     @break
							...
							@if ($users == 'Guru' && $job_title == 'Guru Piket')
								@switch($kehadiran->verifikasi_guru_piket)
							    @case('Belum Di Verifikasi')
							        <button id="verifikasi_guru_piket_siswa" class="btn bg-green" data-id="{{ $kehadiran->user_id }}"> <span class="glyphicon glyphicon-ok"> </span> Verifikasi</button>
							     @break

							      @case('Sudah Di Verifikasi')
							        <button id="verifikasi_guru_piket_siswa" class="btn bg-red" data-id="{{ $kehadiran->user_id }}"> <span class="glyphicon glyphicon-remove"> </span> Batalkan</button>
							     @break
							...
							<button id="editKehadiran" class="btn bg-blue" data-id="{{ $kehadiran->user_id }}" data-toggle="modal" data-target="#editKehadiranSiswa"> <span class="glyphicon glyphicon-edit"> </span> edit </button>
						</td>
					</tr>
					@endforeach
	...
	['pagination' => $kehadiran_siswa, 'tabel' => 'Siswa'])
</div>
